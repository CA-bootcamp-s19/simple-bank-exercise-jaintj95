...
contract SimpleBank {
    ...
    function deposit() public payable returns (uint) {
        /* Add the amount to the user's balance, call the event associated with a deposit,
          ...
          address customer = msg.sender;
          require(enrolled[customer] == true, 'Customer is not enrolled with the bank');
          balances[customer] += msg.value;

          emit LogDepositMade(customer, msg.value);
    ...
    function withdraw(uint withdrawAmount) public returns (uint) {
        /* If the sender's balance is at least the amount they want to withdraw,
           ...
           address customer = msg.sender;
           require(withdrawAmount <= balances[customer], 'Customer is trying to withdraw more balance than that is available');

           balances[customer] -= withdrawAmount;
